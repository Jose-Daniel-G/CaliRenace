...
public class NextLevel8 : MonoBehaviour
{
    // [SerializeField] private Level1 lv1;
    [SerializeField] private GameObject panel;
    [SerializeField] private AudioSource winSound;
    public int levelId = 8;  // ID del nivel completado
    ...
    private void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        {
            int maxScore = 0;
            //     if(lv1.GetScore()> scoreP){maxScore = lv1.GetScore();}else{maxScore = scoreP;}
            // Guarda el nivel completado antes de cambiar de escena
            if (SaveLoadData.Instance != null)
            {
                SaveLoadData.Instance.SaveData(userId, levelId, "8",  maxScore);
